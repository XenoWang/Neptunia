...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 原始之初 | `prototype` | ×3.0 | ×3.0 | ×3.0 | ×3.0 | ×3.0 | 纯测试用 |
| 绀紫之心 | `purple_heart` | ×1.25 | ×1.35 | ×1.3 | ×1.3 | ×1.2 | 均衡 |
| 圣黑之心 | `black_heart` | ×1.4 | ×1.45 | ×1.2 | ×1.15 | ×1.15 | 速攻 |
| 群白之心 | `white_heart` | ×0.75 | ×2.5 | ×1.05 | ×1.45 | ×1.3 | 重装 |
| 翡绿之心 | `green_heart` | ×1.45 | ×1.35 | ×1.5 | ×1.1 | ×1.1 | 速攻 |
| 绀紫妹妹（Nepgear） | `purple_sister` | ×1.3 | ×1.35 | ×1.25 | ×1.25 | ×1.2 | 均衡光剑 |
| 圣黑妹妹（Uni） | `black_sister` | ×1.4 | ×1.3 | ×1.35 | ×1.15 | ×1.15 | 敏捷步枪 |
| 群白妹妹·罗姆（Rom） | `white_sister_rom` | ×0.9 | ×1.4 | ×1.1 | ×1.45 | ×1.35 | 防御冰法 |
| 群白妹妹·拉姆（Ram） | `white_sister_ram` | ×1.05 | ×1.65 | ×1.2 | ×1.25 | ×1.2 | 进攻冰法 |
...
- 悬停高亮并显示完整名字/介绍、点击即选中。八角色（开发者九角色）按可用空间分页，底部箭头翻页；四姐妹立绘待补，暂用初始武器展示占位。
...
| 绀紫妹妹 | 光剑 | `nepgear_sword_tier<N>` | 1.8 → 1.6 |
| 圣黑妹妹 | 步枪 | `uni_rifle_tier<N>` | 2.0 → 1.8（基础射速） |
| 群白妹妹·罗姆 | 冰魔法杖 | `rom_staff_tier<N>` | 1.2 → 1.0（基础射速） |
| 群白妹妹·拉姆 | 冰魔法杖 | `ram_staff_tier<N>` | 1.2 → 1.0（基础射速） |

### 伤害阶梯 / Damage Ladder

下表适用于近战武器。步枪与冰法六阶单发基础伤害为 **5 / 6 / 9 / 13 / 20 / 40**，近战基础伤害为 1。
主手右键发射，每次需 10 可用耐久，无需弹药；所有枪械/法杖共用冷却。冰锥命中附加 2 秒缓慢 I，远程伤害与射速受当前属性修饰器影响。
...
- **完整数值表（54 件）见 `tools/weapon_tiers.csv`**（远程行的“总伤害”指单发基础伤害）。
...
- 四姐妹新增武器使用原生 JSON 立体模型和 Minecraft 内置纹理，模型位于 `models/item/nepgear_sword*`、`uni_rifle*`、`rom_staff*`、`ram_staff*`。六阶用材质区分，后续可由美术替换；下述 PNG 薄板流程适用于原有五位女神。
- 枪弹与冰锥有独立模型，客户端随飞行方向旋转；升级配方和四姐妹正式立绘尚未实现。
